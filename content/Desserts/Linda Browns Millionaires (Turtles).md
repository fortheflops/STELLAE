---
title: Linda Brown's Millionaires (Turtles)
category: Desserts
collection: UK3 Collection
source: Linda Brown
tags:
  - dessert
  - candy
  - turtles
  - chocolate
  - caramel
  - pecans
description: A rich, classic candy featuring chewy caramel and pecan clusters dipped in smooth chocolate. This elegant homemade confection is a timeless holiday favorite.
date: 2026-07-30
draft: false
recipe: "{\"@context\": \"https://schema.org/\", \"@type\": \"Recipe\", \"name\": \"Linda Brown's Millionaires (Turtles)\", \"author\": {\"@type\": \"Person\", \"name\": \"Linda Brown\"}, \"datePublished\": \"2025-10-07\", \"description\": \"A classic recipe for homemade 'Turtles' candy, featuring caramel-pecan clusters dipped in chocolate.\", \"image\": [\"https://cucinamezzaluna.com/Desserts/Confections/Candy/Turtles/Assets/Handwritten_2025-04-17_115722.webp\", \"https://cucinamezzaluna.com/Desserts/Confections/Candy/Turtles/Assets/Handwritten_2025-04-17_120041%20(2).webp\"], \"prepTime\": \"PT30M\", \"cookTime\": \"PT10M\", \"totalTime\": \"PT1H30M\", \"recipeYield\": \"Approx. 3 1/2 dozen\", \"recipeCategory\": \"Candy\", \"recipeCuisine\": \"American\", \"recipeIngredient\": [\"1 (14-ounce) package caramels, unwrapped\", \"3 tablespoons evaporated milk\", \"2 cups pecan pieces\", \"1 (12-ounce) package plus 1/2 cup (18 ounces total) semi-sweet chocolate chips\", \"1-2 tablespoons shortening or coconut oil\"], \"recipeInstructions\": [{\"@type\": \"HowToStep\", \"text\": \"Line a cookie sheet with buttered waxed paper. In a double boiler, melt the caramels with the evaporated milk over low heat, stirring until smooth.\"}, {\"@type\": \"HowToStep\", \"text\": \"Stir in the pecan pieces.\"}, {\"@type\": \"HowToStep\", \"text\": \"Drop the mixture by teaspoonfuls onto the prepared waxed paper and let cool for at least 30 minutes until firm.\"}, {\"@type\": \"HowToStep\", \"text\": \"In a clean double boiler, melt the chocolate chips with the shortening or coconut oil until smooth.\"}, {\"@type\": \"HowToStep\", \"text\": \"Using a spoon and toothpick, dip each caramel-pecan cluster into the melted chocolate to coat. Tap off the excess chocolate.\"}, {\"@type\": \"HowToStep\", \"text\": \"Return the coated candies to the waxed paper and let them cool completely until the chocolate is hard.\"}]}"
---

# Linda Brown's Millionaires (Turtles)

> 📜 **Collection:** UK3 Collection | ✍️ **Attribution:** Linda Brown | 📂 **Category:** Desserts
> *A rich, classic candy featuring chewy caramel and pecan clusters dipped in smooth chocolate. This elegant homemade confection is a timeless holiday favorite.*

---

| Prep Time | Cook Time | Inactive / Chill Time | Yield / Servings |
| :--- | :--- | :--- | :--- |
| 30 mins | 10 mins | 50 mins | 42 Servings |

---

### 🔪 Key Equipment
* Double boiler
* Cookie sheet
* Waxed paper
* Spoon
* Toothpick

---
## Ingredients

| Measurements | Ingredients | Prep / Notes |
| :--- | :--- | :--- |
| 1 (14 oz) pkg | [[Caramels]] | unwrapped |
| 3 tablespoons | [[Evaporated Milk]] |  |
| 2 cups | [[Pecan Pieces]] |  |
| 1 (12 oz) pkg + 1/2 cup (18 oz total) | Semi-Sweet [[Chocolate Chips]] |  |
| 1-2 Tbsps | [[Shortening]] or [[Coconut Oil]] | to replace paraffin |

---

## Instructions

### Step 1: Prepare and Melt Caramel
1. Line a cookie sheet with buttered **waxed paper**. Unwrap **1 (14 oz) package caramels**.
2. In the top of a double boiler, melt the **1 (14 oz) package caramels** with **3 tablespoons evaporated milk** over low heat, stirring constantly until smooth and completely melted.

### Step 2: Form Clusters
3. Remove the caramel mixture from the heat and stir in **2 cups pecan pieces** until evenly coated.
4. Drop the mixture by teaspoonfuls onto the prepared **waxed paper**. Let the clusters cool and set for at least **30 minutes** until firm.

### Step 3: Melt Chocolate and Dip
5. In a clean double boiler, melt **18 ounces semi-sweet chocolate chips** with **1-2 tablespoons shortening or coconut oil** until smooth, glossy, and completely melted.
6. Using a spoon, dip one caramel-pecan cluster at a time into the melted **chocolate**. Use a toothpick to help lift it out, tapping the spoon on the side of the pan to knock off excess **chocolate**.
7. Return the chocolate-coated clusters to the **waxed paper**.
8. Let the candies cool completely until the **chocolate** is hard and set.
9. Store in airtight tin containers with **wax paper** between the layers.

---

> 💡 **Make-Ahead & Storage:** Store in airtight tin containers with wax paper between the layers at room temperature for up to 2 weeks, or freeze for longer storage.

---
## Original Recipe Scan
![Original Handwritten Card](/assets/scans/Handwritten_2025-04-17_115722.webp)
