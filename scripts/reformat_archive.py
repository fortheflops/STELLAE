...
6. "ingredients_sections": Array of sections (e.g., "For the Base", "For the Topping"). If flat, use "Main Ingredients". Each item MUST have "measurement", "ingredient" (wrap key items in wikilinks like [[Potatoes]]), and "notes" (prep state like "diced", "cold", or "divided").
...
Return ONLY a raw JSON object with:
{
    ...
    "author": "Unknown",
    "tags": ["sidedish", "vintage", "potatoes"],
    ...
    "instructions_sections": [
        {
          ...
          "steps": [
            "In a saucepan, combine the **4 cups potatoes** and **water**. Simmer for **20 minutes** until tender."
          ]
        }
    ],
...
def reformat_archive():
    ...
    # THE FAIL-SAFE RELAY RACE: Try Flash first, then fallback models!
    model_fallback_list = ['gemini-3.5-flash', 'gemini-3.5-flash-lite', 'gemini-3.1-flash-lite', 'gemini-2.5-flash']
        ...
        try:
            ...
            while current_model_index < len(model_fallback_list):
                ...
                try:
                    ...
                    error_msg = str(e)
                    if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg or "404" in error_msg or "NOT_FOUND" in error_msg or "503" in error_msg:
...
def save_upgraded_recipe(old_file_path, data):
    ...
    new_filename = safe_title.replace("/", "-").replace("\\", "-").replace("'", "") + ".md"
